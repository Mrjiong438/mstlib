...
#define MST_LIST_NEW_ITEM(_list,_pos,_item) \
	/* TODO custom function to add element */ \
	(_list->item)[_pos] = _item; \
	(_list->length)++; \

#define MST_LIST_REALLOC(_list,n) \
...
//TODO
#define mst_list_expand(_list,_additon_size)
...
#define mst_list_append(_list,_item) \
	do{ \
		...
		if(_list->size == _list->length){ \
			MST_LIST_REALLOC(_list,_list->size * 2) \
		} \
		MST_LIST_NEW_ITEM(_list,_list->length,_item) \
	}while(0) \
...
#define mst_list_insert(_list,_index,_item) \
	do{ \
		if(_index == 0 && _list->length == 0){ \
			...
			MST_LIST_NEW_ITEM(_list,_index,_item) \
			break; \
		} \
